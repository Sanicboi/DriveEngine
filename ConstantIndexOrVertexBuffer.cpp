...
{
	hr = device->CreateCommittedResource(&bufferHeapProperties, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST, NULL, IID_PPV_ARGS(&ptr));
}
...
void ConstantIndexOrVertexBuffer::Copy(ID3D12Device10* device, ID3D12GraphicsCommandList7* list, BYTE* pData, D3D12_RESOURCE_STATES state)
{
	...
	hr = device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_SOURCE, NULL, IID_PPV_ARGS(&tempBuffer));
	if (Ok()) {
